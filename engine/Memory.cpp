...
ResourceHandle ResourcePool::acquire()
{
    if(m_free_index == m_resource_chunks.size())
    {
        parse_chunks(m_allocator->allocate(m_resources_per_pool, m_resource_size));
    }
    ResourceHandle free_index = m_free_indices.back();
    m_free_indices.pop_back();
    return free_index;
}

void ResourcePool::free(ResourceHandle handle)
{
    m_free_indices.push_back(handle);
}

void* ResourcePool::access(ResourceHandle handle)
{
    return m_resource_chunks[handle.index];
}

bool ResourcePool::valid_handle(ResourceHandle handle)
{
    for(ResourceHandle _handle : m_free_indices)
    {
        if(handle.index == _handle.index)
        {
            return false;
        }
    }
