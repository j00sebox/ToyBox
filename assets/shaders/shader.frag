...
void main()
{
    ...
    vec3 l = normalize(direct_light_position - v_position); // light direction
    vec3 v = normalize(camera_data.camera_position - v_position); // view direction
    vec3 h = normalize(l + v); // halfway vector

    vec3 diffuse = max(dot(normal, l), 0.0) * direct_light_colour;

    float shininess = 32.0;
    float strength = 0.2;
    specular_factor = pow(max(dot(normal, h), 0.0), shininess);
    vec3 specular = direct_light_colour * specular_factor * strength;
